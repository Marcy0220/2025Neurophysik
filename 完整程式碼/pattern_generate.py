...
class PatternGenerator:
    ...
    def normalized_complexity(self, pattern, stats=None):
        ...
        if stats:
            entropy_val = (entropy_val - stats['entropy_min']) / (stats['entropy_max'] - stats['entropy_min'] + 1e-6)
        ...
        return 0.5 * entropy_val + structural_score
    ...
    def generate_stat_baseline(self, samples=100):
        entropy_vals = []
...
if __name__ == '__main__':
    SIZE = 8
    generator = PatternGenerator(size=SIZE)

    # Example: Use "十字" pattern as reference
    cross_pattern = np.array([1 if (i % SIZE == SIZE//2 or i // SIZE == SIZE//2) else -1 for i in range(SIZE*SIZE)])
    #test = np.array([1 if i//16 == i%16 else -1 for i in range(256)])
    #print(generator.normalized_complexity(test))

    similar_patterns = generator.generate_similar_complexity_patterns(cross_pattern, count=50, tolerance=0.1)
    print(len(similar_patterns))
    generator.save_patterns(similar_patterns)
    generator.show_patterns(similar_patterns, n=50, save_path="saved_patterns/overview.png")
